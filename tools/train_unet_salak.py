"""
UNet-MobileNetV2 Training Script for Salak Dataset
===================================================

LIGHTWEIGHT ARCHITECTURE:
1. ✅ UNet with MobileNetV2 encoder (~3.5M parameters vs 40M in DeepLabV3+)
2. ✅ Multi-Head diversity (3 heads with ensemble voting)
...
8. ✅ EMA, gradient clipping, cosine LR
9. ✅ NO adversarial discriminator
10. ✅ Faster training & inference

ADVANTAGES:
- Lightweight: 10× fewer parameters than DeepLabV3+
- Faster: ~2× faster training
- Mobile-friendly: Can run on edge devices
- Still accurate: Expected 0.65-0.70 mIoU
...
from model.unetplusplus_mobilenetv2 import UNetPlusPlus_MobileNetV2, MultiHeadUNetPlusPlus_MobileNetV2
...
def get_arguments():
    ...
    # Checkpoints
    parser.add_argument("--checkpoint-dir", type=str, default="./checkpoints_unet")
...
def main():
    ...
    # Create model
    print("\n" + "="*60)
    print("Initializing UNet++ with MobileNetV2 Model...")
    print("="*60)
    
    if args.use_multi_head:
        print(f"  Using Multi-Head UNet++ architecture ({args.num_heads} heads)")
        model = MultiHeadUNetPlusPlus_MobileNetV2(
        ...
        print(f"  Using Single-Head UNet++")
        model = UNetPlusPlus_MobileNetV2(
            n_classes=args.num_classes,
            pretrained=True,
            deep_supervision=False
        )
    ...
    # Training loop
    print("\n" + "="*60)
    print("Starting Training (UNet++ with MobileNetV2)...")
    print("="*60)
    print(f"🔧 UNet++ Features Active:")
    print(f"  • Architecture: UNet++ with MobileNetV2 Encoder")
    print(f"  • Dense Skip Connections: Nested pathways for better fusion")
    print(f"  • Parameters: ~5-7M (7× lighter than DeepLabV3+)")
